...
| 火绒安全 | ⚠️ 暂时不支持 | 仅支持 Windows Defender（自带）|
...
└─────────────────────────────────────────────────────────┘
        ...
        ▼
╔════════════════════════════════════════════════════════╗
║  LAYER 0：Windows Defender AV 扫描（传统病毒/木马）      ║
║  耗时：10-30 秒 | 工具：MpCmdRun.exe（Windows 11/10）     ║
║                                                        ║
║  "C:\Program Files\Windows Defender\MpCmdRun.exe"     ║
║    -Scan -ScanType 3 -File "C:\path\to\skill"         ║
║                                                        ║
║  发现威胁 → ❌ 拒绝，Windows Defender 自动隔离/删除     ║
║  未发现   → ✅ 进入 Layer 1                              ║
╚════════════════════════════════════════════════════════╝
        │
        ▼
...
### 5.1 Windows Defender AV 扫描（Layer 0）

```bash
# 扫描指定技能目录（Windows 路径格式）
"C:\Program Files\Windows Defender\MpCmdRun.exe" -Scan -ScanType 3 -File "C:\path\to\skill"

# 验证退出码
"C:\Program Files\Windows Defender\MpCmdRun.exe" -Scan -ScanType 3 -File "C:\Windows\System32\notepad.exe"
echo "退出码：%ERRORLEVEL%"
# 预期：0 = 无威胁
```

> 注：`-ScanType 3` 表示完全扫描。扫描完成后退出。退出码 **0 = 无威胁**，非 0 = 发现威胁。Windows Defender 负责传统病毒特征库匹配，与我们的语义扫描完全互补。
>
> **暂时不支持火绒（Huorong）**：当前版本仅支持 Windows 自带的 Windows Defender。如需火绒集成，请提交 Issue。
...
- **安装前必做 Layer 0**：用 Windows Defender 扫描技能目录（自动检测路径）
